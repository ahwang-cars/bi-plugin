...
Recommended: extend the existing `~/.tableau-config.json` with a `redshift` block so both plugins share one creds file. user/password come from the same `connection_credentials` block `tableau-sql-updater` already uses.
